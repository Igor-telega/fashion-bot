...
if not BOT_TOKEN or not AIUTA_API_KEY:
    raise SystemExit("⚠️  Заполни TELEGRAM_BOT_TOKEN и AIUTA_API_KEY в .env")
...
# --- Aiuta helpers ---
def aiuta_headers():
    return {"X-API-Key": AIUTA_API_KEY, "Accept": "application/json"}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Привет! 👋 Пришли фото одежды (на вешалке, на полу и т.д.) — я покажу, как она выглядит на модели.\n"
        "Советы: фото с простым фоном и без складок 😉"
    )
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Обрабатываю в Aiuta... Обычно занимает 15–30 секунд 🕐")
        op = aiuta_poll_operation(operation_id, timeout_sec=240, interval_sec=4)

        if op.get("status") == "FAILED":
            await update.message.reply_text("К сожалению, генерация не удалась 😕. Попробуй другое фото.")
            return

        urls = aiuta_extract_generated_urls(op)
        if not urls:
            await update.message.reply_text("Aiuta не вернула изображений. Попробуй другое фото.")
            return
